class Aggressive:
    ...
    def AddIncomingJobs(self, clock):
        ...
        for i in range(0, len(self._clock_ticks)):
            tick = self._clock_ticks[i]
            if tick == clock:
                already_checked = True
        if already_checked == False:
            self._clock_ticks.append(clock)
            for i in range(0, len(self._jobs)):
                if self._jobs[i].GetArrival == clock:
                    self._jobs[i].SetState("Waiting - Ready To Run")
                    self._jobs[i].SetPriority(0)
                    self._jobs[i].SetTimeSlice(self._zero)
    ...
    def RequeueJobAfterIOBlock(self, job):
        curr_priority = job.GetPriority
        if curr_priority != 0:
